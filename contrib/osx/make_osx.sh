...
PACKAGE=Electrum-Meowcoin
GIT_REPO=https://github.com/Electrum-RVN-SIG/electrum-meowcoin
...
info "Installing dependencies sepcific to meowcoin..."
python3 -m pip install --no-build-isolation --no-dependencies --no-binary :all: \
    --no-warn-script-location \
    -Ir ./contrib/deterministic-build/requirements-meowcoin-pypi.txt \
    || fail "Could not install dependencies from pypi"
python3 -m pip install --no-build-isolation --no-dependencies --no-binary :all: \
    --no-warn-script-location \
    -Ir ./contrib/deterministic-build/requirements-meowcoin-git.txt \
...
info "Creating .DMG"
hdiutil create -fs HFS+ -volname $PACKAGE -srcfolder dist/$PACKAGE.app dist/electrum-meowcoin-$VERSION.dmg || fail "Could not create .DMG"

DoCodeSignMaybe ".DMG" "dist/electrum-meowcoin-${VERSION}.dmg"
