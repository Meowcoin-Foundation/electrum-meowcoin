...
APPIMAGE="$DISTDIR/electrum-meowcoin-$VERSION-x86_64.AppImage"
...
# Note: We skip hash verification for continuous releases as they change frequently
...
"$python" -m pip install --no-build-isolation --no-dependencies --no-binary :all: --no-warn-script-location \
    --cache-dir "$PIP_CACHE_DIR" -r "$CONTRIB/deterministic-build/requirements-meowcoin-pypi.txt"
"$python" -m pip install --no-build-isolation --no-dependencies --no-binary :all: --no-warn-script-location \
    --cache-dir "$PIP_CACHE_DIR" -r "$CONTRIB/deterministic-build/requirements-meowcoin-git.txt"
...
cp "$PROJECT_ROOT/electrum-meowcoin.desktop" "$APPDIR/electrum-meowcoin.desktop"
